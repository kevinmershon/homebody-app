...
- A Meta Quest 3 with the Homebody APK (*link: TBD*) on the same Wi-Fi network
...
3. Start everything and put on your headset:
   ```
   docker compose up -d
   ./bin/homebody-server-mac        # macOS -- leave this running
   ./bin/homebody-server-linux      # Linux (coming soon)
   bin\homebody-server.exe          # Windows (coming soon)
   ```
...
| `resources/animations/` | Idle/talking/sit poses (`.hbanim`) — download from Discord or record your own in-headset |
